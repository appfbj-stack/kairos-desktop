...
import { z } from 'zod';
import { getMemoryRepository, type Entity, type EntityType } from '../../infrastructure/memory/sqlite.repository.js';
...
export class SearchEntitiesUseCase {
  async execute(input: SearchEntitiesInput): Promise<Entity[]> {
    const parsed = SearchEntitiesInputSchema.parse(input);
    const repo = getMemoryRepository();

    if (parsed.query) {
      let results = repo.searchEntities(parsed.query, parsed.limit);
      if (parsed.type) results = results.filter((e) => e.type === parsed.type);
      if (parsed.tag) results = results.filter((e) => e.tags.includes(parsed.tag!));
      return results;
    }

    let results = repo.listEntities({ type: parsed.type as EntityType | undefined, limit: parsed.limit });
    if (parsed.tag) results = results.filter((e) => e.tags.includes(parsed.tag!));
    return results;
  }
}
