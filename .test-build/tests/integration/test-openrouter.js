...
async function main() {
    ...
    check('OpenRouter isConfigured=true', openRouter?.isConfigured === true, `isConfigured=${openRouter?.isConfigured}`);
    // 2. List models
    console.log('\n2. Listagem de modelos:');
    const models = await openRouter.models;
    // (ja estao como array)
    const modelList = openRouter.models;
    ...
    const hasGPT4oMini = allModels.some((m) => m.id?.includes('gpt-4o-mini'));
    ...
    try {
        ...
        check('Chat streaming executou', false, err.message);
    }
    ...
    try {
        ...
        check('Tool calling executou', false, err.message);
    }
    // Resumo
