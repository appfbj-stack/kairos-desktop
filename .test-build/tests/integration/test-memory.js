...
async function main() {
    ...
    check('Export tem entities', Array.isArray(exported.entities) && exported.entities.length >= 3);
    check('Export tem conversations', Array.isArray(exported.conversations) && exported.conversations.length >= 1);
    check('Export tem audit', Array.isArray(exported.audit));
    check('Export tem version', exported.version === '0.1.0');
    // 9. LGPD delete all
    console.log('\n9. LGPD delete all:');
    repo.deleteAll();
